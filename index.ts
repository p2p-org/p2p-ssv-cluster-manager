...
app.get(
  '/01-transfer-ssv-tokens-from-factory-to-clusters',
  async (req: Request, res: Response) => {
    logger.info('01-transfer-ssv-tokens-from-factory-to-clusters started')
    res.send(
      '01-transfer-ssv-tokens-from-factory-to-clusters started at ' +
...
app.get(
  '/02-remove-exited-validators-from-clusters',
  async (req: Request, res: Response) => {
    logger.info('02-remove-exited-validators-from-clusters started')
    res.send(
      '02-remove-exited-validators-from-clusters started at ' +
        new Date().toISOString(),
    )
    logEnv()
...
app.get(
  '/03-withdraw-excess-tokens-from-clusters',
  async (req: Request, res: Response) => {
    logger.info('03-withdraw-excess-tokens-from-clusters started')
    res.send(
      '03-withdraw-excess-tokens-from-clusters started at ' +
        new Date().toISOString(),
    )
    logEnv()
