...
export async function removeExitedValidatorsFromClusters() {
  ...
  for (const proxy of proxies) { // iterate proxies
    const clusterKeys = Object.keys(proxiesToClustersWithPubkeys[proxy])

    for (const clusterKey of clusterKeys) { // iterate clusters
      const pubkeysToRemove: string[] = []

      for (const operatorIdsWithPubkey of proxiesToClustersWithPubkeys[proxy][clusterKey]) {  // iterate pubkeys
      ...
      logger.info('For ' + proxy + ' ' + clusterKey + ' ' + pubkeysToRemove.length + ' pubkeys to remove')
